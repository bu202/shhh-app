...
| High (P1) | 13 / 16 | **3 — 초대 코드 동시성(미룸) · 계정 삭제 중간 실패(미확인) · 옛 엣지 캐시(만료 대기)** |
| Medium (P2) | 5 / 5 | 0 |
...
공개 베타를 막는 것은 이제 P0 가 아니라 이 셋이다: **초대 코드 동시성**(사용자당 활성 코드 1개를
DB 가 강제하지 않는다) · **계정 삭제 중간 실패**(중간에 끊기면 무엇이 남는지 미확인) ·
**개인정보 법률 검토**. WAF 부재(볼류메트릭 방어 없음)도 그대로다.
