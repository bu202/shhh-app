...
- **4단계는 로컬 구현 완료다. 원격은 안전 동기화만 실행됐다** — 배포 `19e69dee`(계정 라우트를 닫아
  둔 세대) · `READY_KEY` 등록 · 옛 배포 15개 삭제 · preview `8e16c92e`. **계정 인프라는 미실행** — 원격 D1 쓰기 0건 ·
...
2026-08-22 실측: 옛 배포 15개 전부 `GET /api/book` → **401**. 401 은 「DB 를 조회하고 나서」
나오는 답이다 — 계정 라우트가 열려 있었다는 뜻이다.
