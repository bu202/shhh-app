...
> ✅ **2026-08-22 실행 완료.** 배포 `19e69dee`(source `7477867`) · `READY_KEY` 등록 ·
...
- ⛔ **제어면 삭제와 공개 URL 폐쇄는 다른 사건이다.** 지운 15개의 `<해시>.shhh-app.pages.dev` 는
  **아직 `/api/book` 에 401 로 답한다** — 공개 URL 폐쇄는 **미완료**이고 §16 이 재측정 방법을 적는다.
- ⛔ **배포된 source 는 `7477867` 이고 그 뒤 로컬 커밋은 전부 미배포다.** 위협 57~63 의 수정은 로컬에만 있고
