...
> | 3단계 기술 상세 설계 | **완료(8판)** → `docs/STAGE3_SIGNUP_SECURITY_DESIGN.md`. 6판·7판의 완료 판정은 각각 다음 재감사로 **철회**됐고, 8판이 위협 48~51 · T61~T64 를 더해 다시 닫았다(§0-8) |
> | 4단계 코드·DB 구현 | **로컬 구현 완료 2026-08-20 · 원격 미반영 · 배포 0건.** 전역 user-data drain 은 결정 A′ 로 구현됐다 — **HTTP 요청과 정리 크론 둘 다**(크론은 2026-08-18 에 편입했다). **자체 재감사(위협 39~42)** 에 이어 **독립 재감사가 우회 5건을 더 찾았고 전부 닫았다**: `restore_closed → maintenance` 옆문(43) · 호출자 함수가 정한 삭제 증거(44) · 안 세어진 pending(45) · 잘못된 키로 살아 있는 계정 승격(46) · 인증 전 쓰기 증폭(47). ⚠️ **다섯 건은 그때의 스위트 22개가 전부 통과하는 상태에서 재현됐다** — `scripts/test-reaudit.mjs`(R1~R5 · T56~T60). **2026-08-20 마감 감사가 또 4건을 찾았다**: stale 표식 제거의 경합(48) · `drained:true` 뒤의 주 D1 쓰기(49) · 엣지 방어 없이 열린 계정 경로(50) · 삭제 키 어긋남이 readiness 에 안 나옴(51) — `scripts/test-stage34-closeout.mjs`(T61~T64). 그때 스위트는 **23개가 전부 통과**하고 있었다. **`docs/OPS_RUNBOOK.md` 신설** |
> | 남용 방어 | **미구성이고, 그래서 계정 라우트가 fail-closed 다**(2026-08-20 · 위협 50). Pages Functions 에는 엣지 레이트리밋 바인딩이 없고 `*.pages.dev` 에는 WAF 규칙을 못 건다. `RL` 바인딩이 없으면 `/api/health`·`/api/ready`·`/api/policies` 만 답한다. 무엇을 붙일지는 **사용자 결정**(`docs/OPS_RUNBOOK.md` §13) |
>
