...
| 옛 배포 15개의 `<해시>.shhh-app.pages.dev` | **전부 401** | **열려 있다.** 401 은 「DB 를 조회하고 나서」 나오는 답이다 |
...
그러므로 지금 사실은 **「지웠다. 그런데 아직 닫히지 않았다」**이고, 둘은 다른 말이다.
다음 사람이 할 일은 하나다:
...
> | 3단계 기술 상세 설계 | **완료 2026-08-22(10판)** → `docs/STAGE3_SIGNUP_SECURITY_DESIGN.md` §12-3-4 · §0-10. 9판은 사용자 결정 0~7 로 닫혔고(위협 52~56), 그 뒤 **공식 4단계 전체 재검증**에서 결함 4건이 더 나와 10판이 됐다(위협 57~60 · T73~T76). 6·7·8판의 완료 판정도 각각 철회된 적이 있다 |
