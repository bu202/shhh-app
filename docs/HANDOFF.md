...
> | 3단계 기술 상세 설계 | **완료 2026-08-22(9판) — 사용자 결정 0~7 확정** → `docs/STAGE3_SIGNUP_SECURITY_DESIGN.md` §12-3-4. 8판의 완료 판정은 남용 방어 재감사로 철회됐다가(위협 52~56) 그 결정으로 다시 닫혔다. 6·7판의 완료 판정도 각각 철회된 적이 있다 |
> | 4단계 코드·DB 구현 | **로컬 구현 완료 2026-08-22 · 원격 미반영 · 배포 0건.** 남용 방어 계약 v2 · **사람 확인**(공개 가입만) · **서명 세션 envelope** 까지 들어갔다(T65~T72). 전역 user-data drain 은 결정 A′ 로 구현됐다 — HTTP 요청과 정리 크론 둘 다 |
