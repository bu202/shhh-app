...
인터넷에 남아 있었다.** 전부 지우고, 브랜치 최신 자리를 안전한 세대로 채웠다.

### 왜 했나 — 삭제 전 실측

| 대상 | `GET /api/book` | 뜻 |
|---|---|---|
| `shhh-app.pages.dev`(프로덕션 `19e69dee`) | **503** | 닫혀 있다(계정 라우트가 DB 앞에서 끊긴다) |
| 옛 배포 15개의 `<해시>.shhh-app.pages.dev` | **전부 401** | **계정 라우트가 열려 있다**(닫힌 세대라면 503). ⚠️ **401 자체는 DB 조회의 증거가 아니다** — 아래 「무엇이 실제 위험인가」 |
| `cf-pages.shhh-app.pages.dev` | **401** | 별칭이 옛 세대(`0ff60ff5`)를 가리키고 있었다 |

즉 주소를 아는 사람은 옛 세대로 그대로 들어갔고, 그 세대에는 **위협 52·56** 이 살아 있다
(공개 `/ready` 가 두 DB 를 조회하고, 계정 경로가 열려 있다).

### 순서

| # | 한 일 | 결과 |
|---|---|---|
| 1 | 현재 HEAD(`7f9078a`)의 **깨끗한 worktree** 생성 | `git worktree add --detach` · 미추적 디렉터리는 따라오지 않는다 |
| 2 | 그 사본에서 `npm test`(25개) · `npm run build` | 통과 · dist **58개** · 내부 파일 **0개** |
| 3 | 프로덕션 source 와 실행 파일 대조 | `7477867 → 7f9078a` 의 차이는 **문서 4개뿐**이고 배포되는 것은 없다 |
| 4 | `cf-pages` 브랜치에 **안전 preview** 배포 | **`8e16c92e`** · `--commit-hash 7f9078a…` · `--commit-dirty=false` |
| 5 | 새 preview 두 주소 검증 | `8e16c92e.…` 와 `cf-pages.…` 둘 다: 계정 API **503** · 공개 `/ready` **진단 없음** · `/api/health` `providers: []` · 없는 주소 **404** · 내부 파일 **SPA 폴백**(9,994B · sha256 `7d809fa2268d…`) |
| 6 | 옛 배포 **15개 삭제** | 전부 성공. 남은 것은 `19e69dee`(production)와 `8e16c92e`(preview) **둘뿐** |
| 7 | 삭제 후 목록 재조회 | 2개 |

**삭제한 15개**: `f72f5225` · `acdecfa2` · `fa7d8ef0` · `774015e9` · `ac4136b9` · `2afaf4d5` ·
`018d2bb7` · `0ff60ff5` · `af8ae410` · `30942b87` · `49dd29a4` · `7eca5ba9` · `03370855` ·
`61860f8f` · `552adc79`.

### ⚠️ 함정 — `wrangler pages deployment delete` 는 비대화형에서 **아무것도 안 지운다**

확인 프롬프트의 비대화형 기본값이 **「아니오」**다. 15번을 돌려도 전부 조용히 넘어가고
**종료 코드는 0** 이다(「Using fallback value in non-interactive context: no」가 출력에 섞여 있다).
`--force` 는 **활성 별칭에만** 쓰이는 플래그라 이 프롬프트를 건너뛰지 않는다.
pty 를 붙여 `y` 를 먹이는 방법도 실패했다(입력이 닫히면 취소로 읽힌다).
**실제로 지운 것은 Cloudflare REST API** 다 — 절차는 `docs/OPS_RUNBOOK.md` §16.

⚠️ **「명령이 성공했다」를 「지워졌다」로 읽지 않는다.** 목록을 다시 받아 개수를 세는 것이 근거다.

### 무엇이 실제 위험인가 — 소스로 판정했다 (2026-08-22 정정)

**401 은 「계정 라우트가 열려 있다」까지만 말한다.** 그 뒤에 저장소를 만졌는지는 응답으로
구분되지 않으므로 **각 세대의 소스를 직접 봤다.**

| 세대 | 배포 | 인증 자리 | 자격증명이 **없을 때** | 자격증명이 **있을 때**(위조 포함) |
|---|---|---|---|---|
| D1 세대 | `f72f5225`·`acdecfa2`·`fa7d8ef0`·`774015e9` (source `cba3d3a`·`586cc86`·`468d858`·`cd767ee`) | 쿠키 `shh_s` → `whoAmI()` | `if (!token) return null` — **D1 을 안 만진다** | `sessions ⋈ users` 질의 **1회/요청** |
| KV 세대 | 나머지 11개 (source `e2d89c5`·`436f42b`·`f4752f0`) | `Authorization: Bearer` → `KV.get("s:…")` | 토큰이 비면 **KV 를 안 만진다** | KV `get` **1회/요청** |

그러므로 정확한 위험 서술은 이것이다 — **계정 라우트가 열려 있고, 위조 자격증명 하나당
저장소 읽기 1회가 나가며 요청 수에 상한이 없다.** 「401 이니 DB 를 조회했다」가 아니다.
(2026-08-22 읽기 전용 확인: 위조 쿠키·위조 Bearer 둘 다 401 이라 **응답으로는 구분되지 않는다** —
그래서 소스가 근거다.)

### ⛔ 열려 있는 것 — 지운 주소가 아직 답한다

삭제는 **컨트롤 플레인에서 성공했다**: 목록에 없고, 개별 조회는 `8000009 … does not exist` 다.
그런데 삭제 뒤 **45분 이상** `<해시>.shhh-app.pages.dev` 가 여전히 **401** 을 준다.

- 캐시가 아니다 — 응답에 `cf-cache-status` 가 없고 `cache-control: private, no-store` 다.
- 대조군도 잡았다 — **존재한 적 없는** 해시(`deadbeef.shhh-app.pages.dev`)는 Cloudflare 404 페이지다.

**2026-08-22 16:30 KST 재측정(읽기 전용 GET)** — 삭제 뒤 약 100분:

| 대상 | `/api/book` | 크기 | `cache-control` | `cf-cache-status` |
|---|---|---|---|---|
| 삭제한 15개 전부 | **401** | 37B | `private, no-store`(10개) · 헤더 없음(5개) | **없음** |
| `deadbeef`(존재한 적 없음 · 대조군) | **404** | 16,140B | — | — |
| `19e69dee`(production) | **503** | 75B | `private, no-store` | 없음 |
| `8e16c92e`(preview) | **503** | 75B | `private, no-store` | 없음 |
| `shhh-app.pages.dev` · `cf-pages.…` | **503** | 75B | `private, no-store` | 없음 |

배포 목록도 읽기 전용으로 다시 받았다 — **2개**(`19e69dee` Production · `8e16c92e` Preview).

> ### 당시 판정 — **2026-08-22 에는 공개 URL 이 401 로 남아 있었다**
>
> 그날의 판정은 **「제어면 삭제는 ✅ 완료 · 공개 URL 폐쇄는 ❌ 미완료」** 였고, 둘은 다른 말이다.
> **이 둘을 같은 완료 표시로 합치지 않는다** — 그 규칙은 지금도 유효하다.
> 그날 아직 쓰지 않기로 한 표현: 「옛 배포 폐쇄 완료」 · 「옛 배포 차단 완료」 · 「복원 금지 조건 ⑦ 충족」.
>
> 그날 적은 다음 단계는 **Cloudflare 지원 문의**였고, 그 작업에서는 제출하지 않았다.
> **그 계획은 실행되지 못했다** — 2026-08-23 에 확인해 보니 **무료 플랜에는 기술 지원 티켓 경로가
> 아예 없다**(공식 문서: 청구·계정·등록기관 문의만 케이스를 열 수 있다). 그래서 지원 문의 대신
> **Pages 프리뷰 액세스**로 공개 접근을 막았다.
>
> ⚠️ **현재 상태는 이 절이 아니라 문서 맨 앞의 「2026-08-23 운영 반영 기록 — 옛 배포의 공개 접근을
> Access 로 막았다」를 따른다.** 아래 실측값·삭제 과정·실패 기록은 **그때의 기록**으로 보존한다.

당시 적어 둔 재측정 절차는 지금도 그대로 쓴다(다만 지금 기대값은 401 이 아니라 302 다):

```bash
for d in f72f5225 acdecfa2 fa7d8ef0 774015e9 ac4136b9 2afaf4d5 018d2bb7 \
         0ff60ff5 af8ae410 30942b87 49dd29a4 7eca5ba9 03370855 61860f8f 552adc79; do
  printf '%s ' $d
  curl -s -o /dev/null -w '%{http_code}\n' https://$d.shhh-app.pages.dev/api/book
done
```

**404 가 되기 전까지 옛 세대는 인터넷에 남아 있다고 본다**(2026-08-23 현재도 404 가 아니다 —
Access 뒤에 있을 뿐이다). 옛 엣지 캐시 7개를 닫을 때와
같은 규칙이다 — **만료 예상 시각이 아니라 실제 응답이 근거다.**

### 이번 배포로 열리지 않은 것

계정·가입·로그인 전부. preview `8e16c92e` 는 프로덕션과 **실행 파일이 같고**, 목적은
「브랜치 최신 자리를 안전한 세대로 채워 옛 배포를 지울 수 있게 하는 것」 하나다.
...
| # | 한 일 | 결과 |
|---|---|---|
| 1 | 배포 전 라이브 실측 | ⛔ `/api/book` **401**(계정 경로가 열려 있었다 — ⚠️ 401 자체는 DB 조회의 증거가 아니다) · 키 없는 `/api/ready` 가 **진단을 그대로 노출**(`db:true`) · `/api/policies` **404**(4단계 코드가 안 나가 있었다) |
...
> | 3단계 기술 상세 설계 | **완료 2026-08-22(11판)** → `docs/STAGE3_SIGNUP_SECURITY_DESIGN.md` §12-3-4 · §0-10 · §0-11. 9판은 사용자 결정 0~7 로 닫혔고(위협 52~56), 전체 재검증 4건이 10판(위협 57~60 · T73~T76), 그 뒤 **독립 검토**에서 셋이 더 나와 11판이 됐다(위협 61~63 · T77~T79 · §0-11) — **제공자별 부분 시크릿** · **비밀값 비교가 런타임 timing-safe API 미사용** · **돌연변이 검증이 재현 불가능**. 6·7·8·9·10판의 완료 판정이 각각 철회된 적이 있다 |
> | 4단계 코드·DB 구현 | **로컬 구현 완료 2026-08-22.** **원격은 두 가지로 갈라 읽는다**(2026-08-22): ✅ **안전 동기화는 실행됐다** — production 배포 `19e69dee`(source `7477867`) · `READY_KEY` 등록 · 옛 배포 **15개 삭제** · 안전 preview `8e16c92e`. ⛔ **계정 인프라는 하나도 안 했다** — 원격 `0005` 미적용 · ledger D1 미생성 · `LEDGER` 미바인딩 · 가입·삭제·세션·Turnstile 시크릿 미등록 · OAuth 시크릿 미등록 · 정리 Worker 미배포 · 도메인·WAF 없음. 그래서 라이브의 계정 라우트는 **두 DB 를 만지기 전에 503** 이다. 남용 방어 계약 v2 · **사람 확인**(공개 가입만) · **서명 세션 envelope** 까지 들어갔다(T65~T72). 전역 user-data drain 은 결정 A′ 로 구현됐다 — HTTP 요청과 정리 크론 둘 다 |
...
`npm test` **26개 스위트** 전부 통과한다(2026-08-22 실측, exit 0). 단, **테스트 통과는 보안 완료가 아니다** —
...
npm test                              # 26개 스위트 (빌드·dist·workerd 검사 포함)
