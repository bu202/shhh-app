...
| 6 | 빌드·배포 | `npm test` 21개 스위트 통과 · `npm audit` 0건 · `test-dist` 통과(dist 51개, 내부 파일 0개) |
| 7 | 배포 후 검증 | 아래 |
...
> | 3단계 기술 상세 설계 | **완료(5판)** → `docs/STAGE3_SIGNUP_SECURITY_DESIGN.md` |
> | 4단계 코드·DB 구현 | **로컬 구현 완료 2026-08-18 · 원격 미반영 · 배포 0건.** 전역 user-data drain 은 결정 A′ 로 구현됐다 |
>
...
`npm test` **21개 스위트** 전부 통과한다(2026-08-18 실측, exit 0). 단, **테스트 통과는 보안 완료가 아니다** —
...
| 테스트 | **21개 스위트** 통과(2026-08-18 실측, exit 0). 4단계에서 `test-policies` · `test-signup` · `test-deletion-ledger` · `test-cleanup` 이 늘었다. 개수의 원본은 `package.json` 의 `test` 스크립트이고 `scripts/test-docs.mjs` 가 거기서 읽어 문서와 대조한다 |
| 배포본 | `dist/` **57개 파일**, 내부 파일 0개, 선캐시 **23개**, 캐시 이름 `shhh-v11-df98add5119f` (2026-08-18 빌드). 늘어난 6개는 `policies/`(불변 정책 문서 4 + manifest + index) |
...
| 정리 크론 | **로컬 구현만.** `worker/cleanup/` 에 있고 `wrangler.jsonc` 의 `database_id` 가 `<TODO>` 다 — **가짜 id 를 채워 넣지 않았다.** 배포 전까지 운영에서는 아무도 만료 데이터를 안 치운다 |
...
| 4단계(구현) | **로컬 구현 완료 2026-08-18 · 원격 미반영.** 코드·DB·테스트는 전부 로컬이고 **배포 0건 · 원격 D1 쓰기 0건 · push 0건**이다. **전역 drain 은 방식 미선택** |
...
| ~~4단계 회원가입 구현~~ ✅(로컬) | 가입 UI·API·`policy_events`·`consumed_signup_states`·삭제 표식 ledger·유지보수 3상태·정리 Worker·테스트 4종 | **로컬 완료 2026-08-18.** 남은 것은 **전역 drain 방식 선택**과 **원격 반영 전부**(§4-7) |
| **4단계 전역 user-data drain** | 설계서 §10-9-8 의 후보 A/B/C 를 비교해 하나를 고르고, T6 의 기대값을 「거부된다」로 바꾼다 | **사용자 결정 대기.** 비교표는 설계서 §10-9-9. 이것이 끝나도 §10-8-0 의 나머지 조건(옛 배포 차단 D1~D12 · T8)이 남아 **주 D1 복원 금지는 그대로다** |
...
| 6 | 정리 Worker 배포(`worker/cleanup/wrangler.jsonc`) | `wrangler delete` | 새 배포 대상 + 크론 |
...
npm test                              # 21개 스위트 (빌드·dist 검사 포함)
npm audit
```
