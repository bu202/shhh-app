...
| 6 | 빌드·배포 | `npm test` 22개 스위트 통과 · `npm audit` 0건 · `test-dist` 통과(dist 51개, 내부 파일 0개) |
| 7 | 배포 후 검증 | 아래 |
...
> | 3단계 기술 상세 설계 | **완료(6판)** → `docs/STAGE3_SIGNUP_SECURITY_DESIGN.md` |
> | 4단계 코드·DB 구현 | **로컬 구현 완료 2026-08-18 · 재감사 결함 4건 수정 2026-08-19 · 원격 미반영 · 배포 0건.** 전역 user-data drain 은 결정 A′ 로 구현됐다 — **HTTP 요청과 정리 크론 둘 다**(크론은 2026-08-18 에 편입했다). **2026-08-19 에 닫은 넷**: `LEDGER` 미바인딩 fail-open(위협 39) · 없는 주소·로그인 시작의 쓰기 증폭(위협 40) · 미등록 리미터 버킷(위협 41) · 복원 사전점검이 인자로 우회됨(위협 42). **`docs/OPS_RUNBOOK.md` 신설** |
>
...
`npm test` **22개 스위트** 전부 통과한다(2026-08-18 실측, exit 0). 단, **테스트 통과는 보안 완료가 아니다** —
...
| 테스트 | **22개 스위트** 통과(2026-08-18 실측, exit 0). 4단계에서 `test-policies` · `test-signup` · `test-deletion-ledger` · `test-cleanup` 이 늘었다. 개수의 원본은 `package.json` 의 `test` 스크립트이고 `scripts/test-docs.mjs` 가 거기서 읽어 문서와 대조한다 |
...
| 정리 크론 | **로컬 구현만.** `worker/cleanup/` 에 있고 `wrangler.jsonc` 의 `database_id` 가 `<TODO>` 다 — **가짜 id 를 채워 넣지 않았다.** 배포 전까지 운영에서는 아무도 만료 데이터를 안 치운다. 실패·경보는 2026-08-18 에 마감했다: 실패한 회차는 `ctx.waitUntil()` Promise 를 **거부해** Cron Trigger 에 실패로 남고, 확정 안 된 삭제 표식·연속 실패 3회는 `/api/ready` 의 **`cleanupAlert`** boolean 으로 나온다(개수·오류 문자열은 비공개 · 못 읽으면 `true`). **외부 알림(Slack·이메일 등)은 붙이지 않았다** — 운영 반영 단계의 별도 결정이다 |
...
npm test                              # 22개 스위트 (빌드·dist 검사 포함)
npm audit
```
