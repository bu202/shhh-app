...
> | 3단계 기술 상세 설계 | **완료(4판)** → `docs/STAGE3_SIGNUP_SECURITY_DESIGN.md` |
> | 4단계 코드·DB 구현 | **미착수. 코드 0줄** |
>
> 결정 1·2·5(조건부)·6 은 확정(**결정 2 는 4판에서 의미 정정** — §17), **결정 3·4 는 외부 법률
> 검토(L1~L12)에 종속**되어 있다.
...
| **개인정보 법률 검토** | 처리 근거(제15조 제1항 제1호 vs 제4호) · 국외 이전(제28조의8 제1호 vs 제3호) · 만 14세 · 정책 이벤트 보존 · 삭제 표식의 성격과 보유기간(L9-1~L9-6) · **정책 행위 시각(L11)** · **가입 state 소비 표식(L12)** | **법률 판단은 AI 가 하지 않는다.** 전달 자료는 `docs/PRIVACY_LEGAL_REVIEW_PACKET.md`(**L1~L12**). 사용자 몫 |
| ~~3단계 기술 상세 설계~~ ✅ | **완료 2026-08-17(4판)** → `docs/STAGE3_SIGNUP_SECURITY_DESIGN.md`. 설계가 끝났다는 뜻이고 **구현·법률·출시는 각각 별개다.** 4판이 닫은 것: **가입 state 재사용에 의한 정책 증거 다중 귀속**(위협 34) · **실패한 삭제의 지연 재실행**(위협 35) | — |
...
| **4단계 회원가입 구현** | 가입 UI·API·`policy_events`·**`consumed_signup_states`**·**전역 write drain**·`test-signup`·`test-policies`·`test-deletion-ledger`(명세는 설계서 §13-5 의 **T1~T39**) | 위 결정 + 법률 근거 확정. **먼저 만들면 재생성 migration 을 쓰게 된다**(SQLite 는 `CHECK` 를 `ALTER` 로 못 바꾼다) |
