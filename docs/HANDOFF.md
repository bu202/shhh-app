...
그러므로 지금 사실은 **「제어면 삭제는 ✅ 완료 · 공개 URL 폐쇄는 ❌ 미완료」**이고, 둘은 다른
말이다. **이 둘을 같은 완료 표시로 합치지 않는다.** 그래서 다음 표현을 아직 쓰지 않는다 —
「옛 배포 폐쇄 완료」 · 「옛 배포 차단 완료」 · 「복원 금지 조건 ⑦ 충족」.

⚠️ **다음 단계는 Cloudflare 지원 문의가 필요하다**(제어면에서 지워진 배포가 계속 응답한다).
이번 작업에서는 **문의를 제출하지 않았다** — 별도 승인 대상이다.
...
**404 가 되기 전까지 옛 세대는 인터넷에 남아 있다고 본다.** 옛 엣지 캐시 7개를 닫을 때와
