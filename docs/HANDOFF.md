...
> 지금 기준: 라이브 = 배포 `acdecfa2`(source `586cc86`). 저장소는 그보다 앞선다.
> `CLAUDE.md` 5절이 원본이다.
>
...
>
> **2026-08-17 단계 현황** — 단계 정의의 원본은 `CLAUDE.md` §1-1 이다.
>
> | 단계 | 상태 |
> |---|---|
> | 1단계 보안 기준선 | **로컬 완료 · 운영 미반영**(2026-08-16 작업분이 아직 배포 전 — §4-2b) |
> | 2단계 정책 결정 | **완료 · 외부 법률 검토 미완료** → `docs/STAGE2_ACCOUNT_PRIVACY_DECISIONS.md` |
> | 3단계 기술 상세 설계 | **완료(3판)** → `docs/STAGE3_SIGNUP_SECURITY_DESIGN.md` |
> | 4단계 코드·DB 구현 | **미착수. 코드 0줄** |
>
> 결정 1·2·5(조건부)·6 은 확정, **결정 3·4 는 외부 법률 검토(L1~L11)에 종속**되어 있다.
...
| 테스트 | 16개 스위트 통과(2026-08-17 실측). `test-client` 64개, `test-friends` 156개, `test-auth` 24개, `test-migrations` **4개** 이전 |
...
| **개인정보 법률 검토** | 처리 근거(제15조 제1항 제1호 vs 제4호) · 국외 이전(제28조의8 제1호 vs 제3호) · 만 14세 · 정책 이벤트 보존 · 삭제 표식의 성격 · **정책 행위 시각(L11)** | **법률 판단은 AI 가 하지 않는다.** 전달 자료는 `docs/PRIVACY_LEGAL_REVIEW_PACKET.md`(**L1~L11**). 사용자 몫 |
| ~~3단계 기술 상세 설계~~ ✅ | **완료 2026-08-17(3판)** → `docs/STAGE3_SIGNUP_SECURITY_DESIGN.md`. 설계가 끝났다는 뜻이고 **구현·법률·출시는 각각 별개다** | — |
| ~~3단계 결정 1·2·5·6~~ ✅ | C안 · 임시 상태 제거 · 별도 D1(조건부, 조건 C1~C8 반영 완료) · saga 실패 응답 + **reconciliation 은 promote-only 이고 활성 deletion lease 0건에서만** | — |
| **3단계 결정 3·4** | 정책 이벤트 법적 분기 · CASCADE 여부 | **법률 검토에 종속.** 그전까지 CASCADE 임시 유지, `policy_events` migration **생성 금지** |
| **4단계 회원가입 구현** | 가입 UI·API·`policy_events`·**전역 write drain**·`test-signup`·`test-policies`·`test-deletion-ledger`(명세는 설계서 §13-5 의 T1~T29) | 위 결정 + 법률 근거 확정. **먼저 만들면 재생성 migration 을 쓰게 된다**(SQLite 는 `CHECK` 를 `ALTER` 로 못 바꾼다) |
| **4단계 전역 write drain** | 설계서 §10-9-8 의 후보 A/B/C 를 **실측 비교**해 하나를 고르고, T1~T8 을 통과시킨다 | 4단계 착수. **이것이 끝나야 §10-8-0 의 주 D1 복원 금지가 풀린다** |
...
로컬 테스트(16개 스위트)만 통과한 상태다. 순서가 중요하다 — 하나만 빠지면 지금보다 나빠진다.
...
> | # | 필요한 것 | 어디 | 없으면 |
> |---|---|---|---|
> | ① | 주 D1 **밖의 삭제 표식**을 복원 전후에 재적용하는 절차 | 요구사항 `docs/STAGE2_ACCOUNT_PRIVACY_DECISIONS.md` §9 · saga·실패 매트릭스·**「주 D1 조건부 복원 11단계」**·**「ledger 복원 9단계」**는 `docs/STAGE3_SIGNUP_SECURITY_DESIGN.md` §10 | 탈퇴자가 되살아난다 |
> | ② | **검증 가능한 전역 write drain** — 주 D1 에 쓰는 **11개 경로 전부**가 빠져나간 것을 질의로 확인 | 전수 표 §10-9-6 · 금지 gate **§10-8-0** · 후보 3안 §10-9-8 | 진행 중이던 쓰기가 복원본에 섞인다 — **로그아웃이 무효가 되고 단어장이 옛 버전으로 되돌아간다** |
>
