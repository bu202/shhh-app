...
>
> **2026-08-17: 2단계(정책 설계) 완료 · 3단계는 「설계 보완 완료, 법률 결정·구현 대기」다** —
> `docs/STAGE2_ACCOUNT_PRIVACY_DECISIONS.md` · `docs/STAGE3_SIGNUP_SECURITY_DESIGN.md`(2판).
> ⚠️ **「3단계 완료」가 아니다.** 2판에서 설계 결함 9건을 고쳤고 결정 1·2·5(조건부)·6 이
> 확정됐지만, **결정 3·4 는 외부 법률 검토(L1~L11)에 종속**되어 있고 코드는 0줄이다.
> 전달 자료는 `docs/PRIVACY_LEGAL_REVIEW_PACKET.md`.
> 회원가입 **구현**은 개인정보 처리의 법적 근거가 확정된 뒤에 시작한다(설계서 §20 착수 조건). 공개 OAuth 활성화와
...
| ~~3단계 상세 설계~~ ✅ | **설계 보완 완료 2026-08-17(2판)** → `docs/STAGE3_SIGNUP_SECURITY_DESIGN.md`. **「3단계 완료」가 아니다** — 법률 결정과 구현이 남았다 | — |
| ~~3단계 결정 1·2·5·6~~ ✅ | C안 · 임시 상태 제거 · 별도 D1(조건부, 조건 반영 완료) · saga 실패 응답 + **reconciliation 은 유지보수 모드 전용** | — |
