...
| 6 | 빌드·배포 | `npm test` 23개 스위트 통과 · `npm audit` 0건 · `test-dist` 통과(dist 51개, 내부 파일 0개) |
| 7 | 배포 후 검증 | 아래 |
...
> | 3단계 기술 상세 설계 | **완료(7판)** → `docs/STAGE3_SIGNUP_SECURITY_DESIGN.md`. 6판 완료 판정은 **독립 재감사로 철회**됐고 7판이 위협 43~47 · T56~T60 을 더해 다시 닫았다(§0-7) |
> | 4단계 코드·DB 구현 | **로컬 구현 완료 2026-08-19 · 원격 미반영 · 배포 0건.** 전역 user-data drain 은 결정 A′ 로 구현됐다 — **HTTP 요청과 정리 크론 둘 다**(크론은 2026-08-18 에 편입했다). **자체 재감사(위협 39~42)** 에 이어 **독립 재감사가 우회 5건을 더 찾았고 전부 닫았다**: `restore_closed → maintenance` 옆문(43) · 호출자 함수가 정한 삭제 증거(44) · 안 세어진 pending(45) · 잘못된 키로 살아 있는 계정 승격(46) · 인증 전 쓰기 증폭(47). ⚠️ **다섯 건은 그때의 스위트 22개가 전부 통과하는 상태에서 재현됐다** — `scripts/test-reaudit.mjs`(R1~R5 · T56~T60). **`docs/OPS_RUNBOOK.md` 신설** |
>
...
`npm test` **23개 스위트** 전부 통과한다(2026-08-18 실측, exit 0). 단, **테스트 통과는 보안 완료가 아니다** —
...
| 테스트 | **23개 스위트** 통과(2026-08-18 실측, exit 0). 4단계에서 `test-policies` · `test-signup` · `test-deletion-ledger` · `test-cleanup` 이 늘었다. 개수의 원본은 `package.json` 의 `test` 스크립트이고 `scripts/test-docs.mjs` 가 거기서 읽어 문서와 대조한다 |
...
| 4단계(구현) | **로컬 구현 완료 2026-08-18 · 원격 미반영.** 코드·DB·테스트는 전부 로컬이고 **배포 0건 · 원격 D1 쓰기 0건 · push 0건**이다. **전역 user-data drain 은 결정 A′ 로 구현됐다** — HTTP 요청과 **정리 크론**이 같은 임차증을 든다(T47b·T47d). 정리 크론의 **실패 전파와 경보**도 마감했다(T49) |
...
npm test                              # 23개 스위트 (빌드·dist 검사 포함)
npm audit
```
