...
| **미배포 로컬 커밋** | ⛔ **production source 는 `7477867` 에 고정돼 있고, 그 뒤 로컬 커밋은 전부 미배포다.** 위협 57~63 의 수정도 로컬에만 있다. ⚠️ **여기에 「최신 커밋」 해시를 적지 않는다** — HEAD 는 커밋할 때마다 움직여서 손으로 유지하면 반드시 낡는다(실제로 한 번 낡았다). 확인은 `git rev-parse HEAD` · `git log -1 --oneline` 이 원본이고, 배포된 지점과의 차이는 `git log --oneline 7477867..HEAD` 로 본다 |
