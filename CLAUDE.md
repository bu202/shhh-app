...
- 🔶 KNN 실손 튜닝 (진행 중)
  ...
  - 계기판 커밋됨(`1ae8b05`): 카메라 상태줄에 `최근접 <자모> d=<거리> (임계 40)` 표시. `KNN_MAX=40`은 임시(같은자모 d~8, 다른자모 d~183 사이 추정치).
  - ⚠️ **LOO 99.2%는 과대평가**: 자모당 8장을 1.8초 정지버스트로 찍어 near-duplicate → 자기 쌍둥이를 찾음(nearestSame 중앙값 0.0). 실제 신호는 클래스간 충돌(min nearestOther 0.4; ㄷ↔ㅌ, ㄷ↔ㅡ; 라이브 ㅏ↔ㅗ d=5.07).
  - **다음 단계(보완안, 사용자 승인됨: 라이브 테스트부터)**:
    1. 라이브 새자세 테스트 → 실제 혼동표 작성(특히 ㅏ↔ㅗ, ㄷ↔ㅌ, ㄷ↔ㅡ, ㅅ). Claude가 KNN 예측+거리 읽어 표 생성.
    2. 실측 새자세 거리로 `KNN_MAX` 재조정(현 40 헐렁, 아마 15~25).
    3. 실패 자모만 **자세 바꿔가며**(위치·거리·각도) 재수집 → 정지버스트 누수 해소.
    4. 하이브리드 폴백 추가(미뤄둠): `const label = (knn == null) ? recognizeSign(lastLm) : knn;`
    5. landmark가 거의 같은 쌍(ㅏ/ㅗ, ㄷ/ㅌ)은 정적 손뼈대의 원천적 한계 → 못 고치면 한계로 기록.
