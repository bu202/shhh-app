...
### 마스터 계정 — 2026-08-08
만든 사람의 계정은 무료 벽(`FREE_LIMIT`)에 안 걸린다. **판단은 서버가 한다** — 로컬(`shh-pro`)에만
두면 폰을 바꾸는 순간 사라져서 새 기기에서 다시 벽에 걸린다(로그인을 붙인 이유와 똑같은 문제다).
...
- `test-friends.mjs` 가 네 가지를 잰다: 목록이 비면 프로 아님 · 목록에 있으면 프로 · 남은 아님 ·
  KV 에 안 굳음.
