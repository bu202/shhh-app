...
| **5. 출시** | 이름·아이콘·개인정보처리방침 → Bubblewrap → Play($25) | 🔶 **매니페스트·아이콘·방침 완료** |
| **6. 카메라 채점** (후순위) | 지화 32자모 점수 + 교정 안내 | ⬜ 뒤로 미룸 |

**다음 할 일**: **5단계 남은 것 — Bubblewrap 빌드 → `.well-known/assetlinks.json` → Play $25.**
1단계는 단어를 넣을 때마다 계속 도는 상시 작업.

### 5단계 — 끝난 것 / 남은 것 (2026-08-06)
| 항목 | 상태 |
|---|---|
| `manifest.webmanifest` (이름 `쉿`, 색을 디자인 토큰에 맞춤, PNG 192·512·maskable) | ✅ |
| `icons/icon-192.png`·`icon-512.png` — `icon.svg` 를 헤드리스 크롬으로 렌더 | ✅ |
| `privacy.html` + 푸터 링크 (Play 등록 필수, 공개 URL 이어야 함) | ✅ |
| `.nojekyll` (GitHub Pages 가 `.well-known` 같은 점 폴더를 서빙하게) | ✅ |
| `.well-known/assetlinks.json` | ⬜ **Bubblewrap 키스토어 SHA-256 이 나온 뒤에** 쓴다. 미리 넣으면 틀린 지문이 박힌다 |
| Bubblewrap init/build (JDK·Android SDK 내려받음) | ⬜ |
