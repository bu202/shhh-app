...
# 🚧 지금 막힌 것 — 새 세션은 **여기부터 보고하고 시작한다**

> 이 블록은 **임시**다. 아래 「지우는 조건」을 다 채우면 이 블록 전체를 CLAUDE.md 에서 삭제한다.
> 남아 있다는 건 아직 안 끝났다는 뜻이다. 세션 시작 시 **먼저 숫자를 다시 재고**(아래 명령) 보고할 것 —
> 이 문서의 숫자를 그대로 믿지 말 것. 데이터가 바뀌면 숫자도 바뀐다.

## 무엇이 문제인가

**표제어 문자열만으로는 어느 수형인지 정해지지 않는다.** 사전에 같은 표제어가 여러 손모양으로 있는데
(`보다` = [시각] / 조사 '~보다'), 앱은 첫 후보를 임의로 집는다.
`보고싶다`에서 실제로 조사 수형이 떴다 — **⛔ "실제 수어만 보여준다" 위반이 화면에서 일어난 사례**다.

사람이 확인한 대장(`ksl-verified.json`)은 `보다@IMG000227009` 표기로 못박아 해결했다(함정 14).
**자동 추출본 `ksl-compounds.json` 2,481건은 그대로다.**

```bash
# 숫자 다시 재기 (읽기만 함, 30초)
node -e '
import("./scripts/_app.mjs").then(({loadApp})=>{
  const M=loadApp("buildIndex, norm, getIdx: () => INDEX");
  const img=require("./data/ksl-dict.json"), fr=require("./data/ksl-fulldict.json");
  const full=Array.isArray(fr)?fr:Object.values(fr)[0];
  const seen=new Set(img.map(e=>e.word)), merged=[...img];
  for(const r of full) if(!seen.has(r.word)){seen.add(r.word);merged.push({...r,media:r.media||{src:[]}});}
  M.buildIndex(merged); const I=M.getIdx();
  const comp=require("./data/ksl-compounds.json");
  let amb=0, diff=0;
  for(const v of Object.values(comp)){
    const c=v.parts.map(p=>I.get(M.norm(p))||[]);
    if(c.some(x=>x.length>1)) amb++;
    if(c.some(x=>new Set(x.map(e=>e.description)).size>1)) diff++;
  }
  console.log(`합성 ${Object.keys(comp).length} · 부품이 여러 후보 ${amb} · 그중 수형이 실제로 다른 것 ${diff} · 안전(설명 동일) ${amb-diff}`);
});'
# 2026-08-05 측정: 합성 2481 · 여러 후보 1682 · 수형이 실제로 다름 1442 · 안전 240
```

## 어디까지 Claude 가 할 수 있나 (2026-08-05 판단)

| 작업 | 가능? | 근거 / 한계 |
|---|---|---|
| 애매한 부품 **찾아내기** | ✅ 완전 자동 | 위 명령. 판단 0, 순수 문자열 비교 |
| 후보 설명이 **똑같은 240건** 못박기 | ✅ 완전 자동 | 어느 쪽을 골라도 **같은 손모양**이라 판단이 필요 없다 |
| **텍스트 교차대조**로 후보 좁히기 | ✅ 모델 없이도 가능 | `보다`를 푼 방법: 같은 수형 설명을 공유하는 다른 표제어를 찾으니 `에서④`(조사)가 나왔다. 그림 판독이 아니라 문자열 비교라서 안전 |
| CSV `수형설명` 재활용 | 🔶 **미확인 — 먼저 이것부터** | 아래 「먼저 할 일」 |
| 모델로 후보 **순위 매기기** | ✅ 가능 (Haiku, 텍스트만) | 1,442건 ≈ **₩3,600**. CLAUDE.md 기준 '선별'이지 '판정'이 아니다 |
| **손 그림 보고** 판정 | ❌ **하면 안 된다** | 지화 ㅗ 오판, "보고싶다=그리워하다" 오판. ⛔ 규칙으로 금지 |
| **유튜브 영상**으로 확인 | ❌ 전수는 불가 | 개별 몇 건은 Chrome 으로 가능하나 느리고 프레임 오독 전례 있음. 병목은 원래부터 **사람이 영상 보는 시간** |
| 최종 승인 (`--add`) | ❌ **사람** | 설계상 막아뒀다(`--add` 는 애매하면 멈춘다) |

## 먼저 할 일 — CSV 확인 (1분, 이게 갈림길)

`~/Downloads/문화체육관광부 국립국어원_한국수어사전_한국어대응표현정보_20240909.csv` 의 **`수형설명`** 컬럼이
**합성어 전체의 손동작**을 담고 있는지 한 행(예 `두통`)만 열어서 본다.

- **담고 있다면** → 부품 후보의 설명과 문자열로 대조해 **판단 없이 기계적으로** 상당수가 풀린다.
  `scripts/build-compounds.mjs` 가 이 컬럼을 버리고 있으니(2026-08-05 에 "앱이 안 읽는다"고 뺐다) 되살린다.
- **아니라면** → 쓸모 없다. 아래 순서로 간다.

## 진행 순서 (권장)

1. CSV `수형설명` 확인 → 되면 기계적 해결부터.
2. 설명이 같은 **240건 자동 못박기**(판단 불필요).
3. **미검증 표시 + 대안 수형 노출** — *판정이 아니라서 Claude 가 전부 할 수 있다.*
   지금 화면은 사람이 확인한 합성과 자동 추출을 구분하지 않고, 부품이 여러 수형이어도 하나만 보여준다.
   이 둘을 넣으면 남은 건이 **"틀린 답"에서 "확인 안 된 후보"로** 바뀐다. 검증 진도와 무관하게 정직해진다.
4. 남은 것은 모델 선별(₩3,600)로 **근거 붙은 목록**을 만들어 사람이 한 건당 몇 초에 판단할 수 있게.
5. 사람이 `--add` 로 대장에 쌓는다(= 1단계 상시 작업).

## 지우는 조건 — 아래가 **전부** 참이면 이 블록을 삭제한다

- [ ] 위 측정 명령의 **`수형이 실제로 다름` 이 0** 이다. (핀을 박았거나, 애매한 합성을 화면에서 뺐거나)
- [ ] 또는: 3번(미검증 표시 + 대안 노출)이 들어가서 **단정하는 화면이 없다**. ← 이쪽만 채워도 ⛔ 위반은 사라진다
- [ ] `node scripts/verify.mjs --check` 통과
- [ ] `for t in book fingers compounds match assemble knn screen home; do node scripts/test-$t.mjs; done` 전부 통과
- [ ] 실제 화면(시뮬레이터 또는 브라우저)에서 애매한 합성 하나를 열어 눈으로 확인

**삭제할 때 같이 할 것**: ① 볼트 노트(`~/Desktop/claude_brain/01-projects/translator.md`) 결정 로그에
**왜 그렇게 정했는지** 한 줄 ② 「함정 14」에 최종 방식 반영 ③ 이 블록 삭제.
...
**다음 할 일**: 맨 위 🚧 블록(동음이의 1,442건)이 먼저다 — ⛔ 위반이 화면에 남은 채로 출시하지 않는다.
그다음 5단계(출시 준비 — 아이콘·개인정보처리방침 → Bubblewrap → Play $25). 1단계는 단어를 넣을 때마다 계속.
