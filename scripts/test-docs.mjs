...
const STAGE_FACTS = [
  ...
  [4, [/로컬 구현 완료/, /19e69dee/, /계정 인프라는? (하나도 )?(안|미)/, /남용 방어|envelope|사람 확인/],
...
//    다음 사람이 한 곳만 고치고 나머지를 낡게 만든다.
{
  ...
  const NINE = [
    [/배포\s*`?19e69dee`?/, "① 안전 동기화 배포(19e69dee)가 실행됐다는 기록"],
    ...
    [/이번[^\n]{0,40}(수정|재검증)[^\n]{0,40}배포하지 않았다|프로덕션에 배포하지 않았다/,
     "⑤ 이번 코드 수정은 production 에 배포하지 않았다는 사실"],
...
// 여기서는 **짝**을 본다.
{
  ...
  for (const [f, t] of all) {
    t.split("\n").forEach((ln, i) => {
      ...
      if (/원격 (반영 )?0건|배포 0건|원격은 아무것도/.test(ln) && /19e69dee|8e16c92e/.test(ln))
...
{
  const UNSHIPPED = /미배포|로컬에만|배포하지 않았다/;
  for (const f of DOCS) {
    R(f).split("\n").forEach((ln, i) => {
      if (!UNSHIPPED.test(ln)) return;
      if (HISTORY_MARK.test(ln)) return;              // 스스로 과거라고 밝힌 줄
      for (const m of ln.matchAll(/위협\s*\*{0,2}(\d+)~(\d+)\*{0,2}/g)) {
        if (+m[2] !== maxTh)
          bad(`${f}:${i + 1} 미배포 범위를 「${m[0].trim()}」이라고 적었다 — `
            + `그 뒤 커밋이 전부 미배포라면 끝은 최신 위협 번호 ${maxTh} 여야 한다\n`
            + `      "${ln.trim().slice(0, 90)}"`);
      }
    });
  }
}
ok(`미배포 범위의 위협 끝번호 == ${maxTh}`);

console.log(fails
  ? `test-docs: 실패 ${fails}건`
  : "test-docs: 통과 — 낡은 문구 · 죽은 § 참조 · 번호 연속성 · 선언된 개수 · 판 번호 · 필수 절 · 완료 범위 · 보유기간 단정 · 스위트 수 · 낡은 운영 상태 · 단계 상태 일치 · 주 D1 접근 분류 등재 · 법률 자료 현재 사실 · 인수인계 현재성 · 현재 상태 구간의 낡은 drain·구현·lease·T6 서술 · drain 미구현 0건 · 정리 대상 개수 = 코드 · 2단계 결정서 현재성 · 재검증 후 현재 사실 9종 · 모순 5종 · 운영현황 실측값 · 날짜별 운영 기록 · 움직이는 해시 · Access 이후 현재형 401 · 돌연변이 개수=MUTATIONS · 미배포 범위의 위협 끝번호");
