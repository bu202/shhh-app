...
const STAGE_FACTS = [
  ...
  [3, [new RegExp(`${EDITION}판`), /미완료|재개|다시 열|결정 대기/],
   [/3단계[^|]*미착수/, new RegExp(`${EDITION - 1}판(으로)? 완료`)],
   `3단계는 ${EDITION}판이 진행 중이고 사용자 결정 전에는 완료가 아니다`],
  // 4단계는 **로컬만** 끝났다. 「구현 완료」가 「출시 가능」으로 읽히지 않게 두 사실을 함께 요구한다.
  [4, [/로컬 구현/, /원격 미반영/, /배포 0건/, /남용 방어/], [/미착수/, /코드 0줄/, /출시 완료/],
   "4단계는 로컬 구현이고 원격은 아무것도 안 했다 — 남용 방어 부분은 9판에서 다시 열렸다"],
];
...
// 면제는 검사 14·15 와 같다 — **같은 줄**의 명시적 역사 표식뿐이다.
{
  ...
  if (i < 0 || j < i) bad(`${STAGE2} 에 현재상태 블록이 없다 — 상단 갈래표가 언제 낡았는지 알 수 없다`);
    ...
    for (const [re, what] of [
      [/원격 반영 0건|원격 미반영|배포 0건/, "원격은 아무것도 안 했다는 사실"],
      [/No-Go/, "출시 판정"],
...
console.log(fails
  ...
  : "test-docs: 통과 — 낡은 문구 · 죽은 § 참조 · 번호 연속성 · 선언된 개수 · 판 번호 · 필수 절 · 완료 범위 · 보유기간 단정 · 스위트 수 · 낡은 운영 상태 · 단계 상태 일치 · 주 D1 접근 분류 등재 · 법률 자료 현재 사실 · 인수인계 현재성 · 현재 상태 구간의 낡은 drain·구현·lease·T6 서술 · drain 미구현 0건 · 정리 대상 개수 = 코드 · 2단계 결정서 현재성");
