...
for (const [f, from, to, label] of CURRENT_REGIONS) {
  ...
  lines.forEach((ln, k) => {
    ...
    if (/T6\s*[·,]\s*T8|T6\s*(?:과|와)\s*T8/.test(ln) && /못 막는|실패 재현/.test(ln))
...
console.log(fails
  ...
  : "test-docs: 통과 — 낡은 문구 · 죽은 § 참조 · 번호 연속성 · 선언된 개수 · 판 번호 · 필수 절 · 완료 범위 · 보유기간 단정 · 스위트 수 · 낡은 운영 상태 · 단계 상태 일치 · 주 D1 접근 분류 등재 · 법률 자료 현재 사실 · 인수인계 현재성 · 현재 상태 구간의 낡은 drain·구현·lease·T6 서술");
