...
// 3c. 분류가 못 거른 무거운 말(2026-08-12). 홈에 '죄수'가 떴다 — 분류는 주제만 적어 둔 것이라
// '일상생활 > 인간' 안에 죄수·감옥이 같이 있다. app.js 의 HEAVY 목록을 지우면 여기서 걸린다.
for (const w of ["죄수", "감옥", "교도소", "사형", "폭행", "백혈병", "치매", "죽다"]) {
  assert.ok(!words.has(w), `'${w}' 가 아직 오늘의 수어 후보에 있다 — HEAVY 목록이 안 걸렸다`);
}
// 지우기만 한 게 아니라는 확인. 일상어가 같이 날아갔으면 필터가 과하다.
// 가벼운 병과 중립적인 직업어까지 빼면 과한 것이다 — 둘이 실제로 주고받는 말이다.
for (const w of ["사랑", "호랑이", "커피", "연애", "감기", "두통", "병원", "경찰", "장애인"]) {
  assert.ok(words.has(w), `'${w}' 가 후보에서 빠졌다 — 필터가 과하다`);
}
