...
// camera.js 실제 features/knnClassify를 백업샘플로 검증(로직 중복 없음).
// camera.js 는 최상위에서 localStorage 를 안 읽으므로 그냥 import 하면 된다(스텁 불필요).
// ⚠️ 샘플 8장이 1.8초 정지버스트라 서로 near-duplicate → LOO는 과대평가. 클래스간 거리가 진짜 신호.
import { readFileSync } from "node:fs";
import assert from "node:assert";
import { features, knnClassify, KNN_MAX, setSamples } from "../js/camera.js";
...
assert.deepStrictEqual(features(toLm(S[0].f)), S[0].f, "features 왕복 불변이 깨짐");

// LOO: 자기 자신 빼고 분류
let ok = 0;
const miss = [];
S.forEach((s, i) => {
  setSamples(S.filter((_, j) => j !== i));
  const p = knnClassify(toLm(s.f));
...
console.log(`다른자모 최근접: ${minOther.toFixed(2)} (${pair}) · KNN_MAX=${KNN_MAX}`);
assert(minOther < KNN_MAX, "임계가 클래스간 거리보다 작음 — 이 임계는 자모 구분용이 아니라 비자모 거부용");

// 임계 거부: 자모가 아닌 손(모든 점이 손목에 뭉친 모양)은 미인식(null)이어야
setSamples(S);
assert.strictEqual(knnClassify(toLm(new Array(42).fill(0))), null, "비자모 손모양이 거부되지 않음");
console.log("✅ 통과");
