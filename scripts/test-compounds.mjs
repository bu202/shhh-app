...
// ── 2026-08-08 일상어 감사. 여기는 **진짜 사전**을 쓰므로 색인·대장 규칙을 직접 잰다.
const cards = (t) => M.matchSentence(t).map((k) =>
  k.type === "entry" ? k.entries[0].word
  : k.type === "compound" ? (k.combo.labels || k.combo.parts).join("+")
  : "?" + k.text);

// 1. 괄호는 뜻이 아니라 동음이의 구분표다((신체의)눈·(마시는)차). 괄호 뗀 형태를 색인에 안 넣으면
//    사용자가 치는 `눈`·`차` 가 사전에 없는 말이 되어 지화로 떨어진다. 32개 표제어가 그랬다.
assert.deepEqual(cards("눈"), ["(신체의)눈"], "눈 → (신체의)눈. 괄호 뗀 형태가 색인에 없다");
assert.deepEqual(cards("차"), ["(마시는)차"], "차 → (마시는)차");
// 다만 **별칭보다 뒤**에 둔다. 앞에 두면 이미 답이 있던 12개의 답이 바뀐다 —
// 좋아지는 것(다리 [남대문]→[(시설물)다리])과 나빠지는 것(닫다 [폐쇄]→[(입을)닫다])이 섞여 있어
// 어느 쪽이 맞는지는 판단이고, 판단은 사람 몫이다. 지금은 아무도 답하지 않던 자리만 채운다.
assert.deepEqual(cards("닫다"), ["폐쇄"], "닫다 → 폐쇄. 괄호 표제어가 이미 있던 답을 밀어냈다");
assert.deepEqual(cards("다리"), ["남대문"], "다리 → 남대문. 괄호 표제어는 뒤 후보로만 붙는다");

// 2. 1글자 가드(함정 16)는 **색인의 조사·어미**를 막으려던 것이지 사람의 판정을 막으려던 게 아니다.
//    대장에 밥=[먹다] 를 적어도 문장 안에서 안 잡히면 대장이 반만 듣는다.
