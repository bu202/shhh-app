...
--    대가로 개인정보를 쌓지 않는다. 만료 행은 **정리 크론**(`worker/cleanup/`)이 지운다 —
--    2026-08-18 이전에는 「다음 로그인」에만 붙어 있어서 아무도 안 오면 영원히 남았다(위협 38).
...
-- ⛔ **2026-08-20 부터 이 표는 쓰이지 않는다.** 남용 방지 카운터는 ledger D1 로 옮겼다
--    (`worker/ledger-schema.sql` · `migrations-ledger/0003` · 설계서 위협 49). 리미터는 임차증보다
--    **먼저** 돌아야 하는데, 그러면 그 쓰기가 임차증 밖이라 「주 D1 은 임차증 안에서만 만진다」에
--    예외가 생긴다 — 그 예외가 `drained:true` 뒤의 주 D1 쓰기를 만들었다.
--    표를 지우는 migration 은 **쓰지 않았다**: 파괴적 원격 작업은 별도 승인 대상이고,
